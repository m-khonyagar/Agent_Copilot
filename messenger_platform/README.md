# Amline Messenger — پیام‌رسان چند‌کاناله املاین

پلتفرم مدیریت پیام‌رسانی چند‌کاناله برای مشاوران و مدیران **Amline** (`amline.ir`) با پنل مدیریت و اپلیکیشن اندروید.

---

## یکپارچه‌سازی با Amline

این پروژه به‌طور مستقیم با API پلتفرم **Amline** (`api.amline.ir`) متصل است:

| قابلیت | توضیح |
|---|---|
| **همگام‌سازی مخاطبین** | کاربران Amline از طریق `GET /admin/users` به مخاطبین محلی وارد می‌شوند |
| **ثبت پیام‌ها در Amline** | هر پیام ارسالی/دریافتی از طریق `POST /users/user-texts` در CRM Amline ثبت می‌شود |
| **احراز هویت OTP** | ورود با OTP Amline از طریق `/admin/otp/send` و `/admin/login` |
